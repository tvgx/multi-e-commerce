...
export class SystemService {
    ...
    validateJsonSetup(jsonString: string) {
        try {
            ...
            parsed.sections.forEach((sec: any, index: number) => {
                if (!sec.id || !sec.type) {
                    throw new Error(`Section at index ${index} is missing 'id' or 'type'.`);
                }
            });

            return { valid: true, message: "JSON is structurally valid for Zero-File Rendering." };
        } catch (error: any) {
            throw new BadRequestException({ valid: false, message: error.message });
        }
    }

    // Mass sync generic template logic
    async massSyncFeature(targetAttr: string, newValue: any) {
