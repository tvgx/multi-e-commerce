...
export class NavigationService {
  ...
  async createMenu(ownerId: string, dto: CreateNavigationDto): Promise<BaseResponseDto<any>> {
    try {
      ...
      throw new InternalServerErrorException('Failed to create navigation menu');
    }
  }

  async getMenuByHandle(handle: string, shopId?: string): Promise<BaseResponseDto<any>> {
    ...
    if (!menu) {
      throw new CustomException(ResponseCodes.NO_DATA_END_OF_LIST, 'Menu not found', HttpStatus.NOT_FOUND);
    }

    return BaseResponseDto.success(menu);
  }

  async updateMenu(ownerId: string, id: string, dto: UpdateNavigationDto): Promise<BaseResponseDto<any>> {
    try {
      ...
      if (!menu) throw new CustomException(ResponseCodes.NO_DATA_END_OF_LIST, 'Menu not found', HttpStatus.NOT_FOUND);
      if (menu.shop.ownerId !== ownerId) throw new CustomException(ResponseCodes.NOT_ACCESS, 'Not access.', HttpStatus.FORBIDDEN);
      ...
      if (error instanceof CustomException) throw error;
      throw new InternalServerErrorException('Failed to update navigation menu');
    }
  }
