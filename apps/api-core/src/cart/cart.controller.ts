...
export class CartController {
  ...
  private extractSessionId(headers: any): string {
    const sessionId =
      headers['x-session-id'] || headers['authorization']?.split(' ')[1];
    if (!sessionId) {
      throw new BadRequestException(
        'Session ID is required (x-session-id header or Bearer token)',
      );
    }
    return sessionId;
  }
  ...
  @Get(':shopId')
  async getCart(@Param('shopId') shopId: string, @Headers() headers: any) {
  ...
  @Delete(':shopId/clear')
  async clearCart(@Param('shopId') shopId: string, @Headers() headers: any) {
    const sessionId = this.extractSessionId(headers);
