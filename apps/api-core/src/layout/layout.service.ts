...
export class LayoutService {
  ...
  async syncLayout(layout: CustomerLayout) {
    ...
    this.logger.log(
      `[LayoutService] Syncing layout for shop: ${layout.shopId}`,
    );
    ...
    this.logger.log(
      `[LayoutService] Layout synced and caches invalidated for ${layout.shopId}`,
    );
  }
  ...
  async getCompiledLayout(shopId: string): Promise<CustomerLayout | null> {
    ...
    const tenantDelta =
      await this.minioService.getLayout<CustomerLayout>(shopId);
    if (!tenantDelta) {
      this.logger.warn(
        `[LayoutService] No layout found in MinIO for shopId=${shopId}`,
      );
      return null;
    }
  ...
  private async triggerNextRevalidate(tag: string): Promise<void> {
    ...
    try {
      ...
      if (!res.ok) {
        this.logger.warn(
          `[LayoutService] Revalidate returned ${res.status} for tag=${tag}`,
        );
      } else {
        this.logger.debug(
          `[LayoutService] Revalidated Next.js cache tag: ${tag}`,
        );
      }
    } catch (err) {
      // Non-fatal — ISR TTL will eventually expire
      this.logger.warn(
        `[LayoutService] Could not reach Next.js for revalidation: ${err}`,
      );
    }
  }
}
