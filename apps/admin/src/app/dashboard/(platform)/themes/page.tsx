    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">{t("themesMarket.title")}</h1>
            <p className="text-sm text-zinc-400">
              {t("themesMarket.subtitle")}
            </p>
          </div>
        </div>
        <button
        ...
        >
          <RefreshCw size={15} /> {t("themesMarket.reload")}
        </button>
      </div>

      <div className="flex gap-2">
        {(["pending", "published"] as ThemeReviewTab[]).map((tabKey) => (
          <button
            key={tabKey}
          ...
          >
            {tabKey === "pending" ? t("themesMarket.tabPending") : t("themesMarket.tabPublished")}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-16 text-center text-zinc-500">
          {tab === "pending"
            ? t("themesMarket.emptyPending")
            : t("themesMarket.emptyPublished")}
        </div>
      ) : (
            ...
            >
              ...
              <div className="p-5 flex flex-col flex-1 gap-2">
                <div className="flex items-center justify-between gap-2">
                  <h2 className="font-semibold text-white">{theme.title}</h2>
                  {theme.category && (
                ...
                {theme.ownerShopId && (
                  <p className="text-xs text-zinc-600">{t("themesMarket.shopLabel")} {theme.ownerShopId}</p>
                )}
                {tab === "pending" && (
                  <div className="mt-3 flex gap-2">
                    ...
                    >
                      <X size={15} /> {t("themesMarket.reject")}
                    </button>
                  </div>
                )}
              </div>
