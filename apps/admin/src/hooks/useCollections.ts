...
export function useCollections(shopId: string) {
  ...
  const fetchCollections = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiClient.get<Collection[]>(`/api/collections?shopId=${shopId}`);
  ...
  const createCollection = async (data: Partial<Collection>) => {
    setError(null);
    try {
      const res = await apiClient.post<Collection>(`/api/collections`, data, { shopId });
  ...
  const updateCollection = async (id: string, data: Partial<Collection>) => {
    setError(null);
    try {
      const res = await apiClient.put<Collection>(`/api/collections/${id}`, data, { shopId });
  ...
  const getCollectionDetails = async (slug: string) => {
    try {
      const res = await apiClient.get<Collection>(`/api/collections/${slug}?shopId=${shopId}`);
      return res.data;
    } catch (err: any) {
      throw err;
    }
  ...
  const addProductsToCollection = async (id: string, productIds: string[]) => {
    try {
      await apiClient.post(`/api/collections/${id}/products`, { productIds }, { shopId });
    } catch (err: any) {
      throw err;
    }
  };
  ...
  const removeProductFromCollection = async (id: string, productId: string) => {
    try {
      await apiClient.delete(`/api/collections/${id}/products/${productId}`, { shopId });
    } catch (err: any) {
      throw err;
    }
  };
